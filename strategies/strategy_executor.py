...
class StrategyExecutor:
    ...
    def dynamic_trend_strategy(self, rets, slow_window=12, fast_window=2):
        ...
        # 计算策略收益率,扣除交易成本
        strategy_rets = np.zeros_like(rets)
        position_change = current_position - previous_position
        strategy_rets[position_change == 0] = rets[position_change == 0] * previous_position[position_change == 0]
        strategy_rets -= self.trading_cost * np.abs(position_change)
        ...
        for date, prev_pos, curr_pos, ret in zip(rets.index, previous_position, current_position, strategy_rets):
            ...
            if position_change != 0 or date == rets.index[-1] or date == rets.index[0]:  # 只记录：有操作的日期、开始日期、结束日期
                operation_date = date + pd.Timedelta(days=1)  # 执行操作的日期为发现信号的第二个交易日
                if position_change > 0:
                    ...
                    operation_type = "检查点"
                self.strategy_operations[operation_date.date()].append(
